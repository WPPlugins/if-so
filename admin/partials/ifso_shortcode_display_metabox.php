...
<p class="php-shortcode-toggle-link">> <?php _e('PHP code (for developers only)', 'if-so'); ?></p>
...
<?php if(false): /*:$post_status != 'publish'):*/ ?>
					<p class="initial-instructions"><b><?php _e('', 'if-so'); ?>3</b> <?php _e('Paste the shortcode in your website', 'if-so'); ?></p>
									<?php endif; ?>
<p><?php _e('The shortcode will be available after publish', 'if-so'); ?></p>
<?php endif; ?>