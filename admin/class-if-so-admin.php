...
class If_So_Admin {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name.'DateTimePickerCss', plugin_dir_url( __FILE__ ) . 'css/jquery.datetimepicker.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		echo "<script>
				...
				jsTranslations['translatable_dupplicated_query_string_notification_trigger'] = '".__('You are already using this query string')."';
				jsTranslations['translatable_dupplicated_query_string_notification_publish'] = '".__('It is not possible to create two query string triggers with the same name. If you publish now the second trigger will be deleted')."';
		</script>";
		//wp_enqueue_script( $this->plugin_name.'BootstrapJS', 'https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js', array( 'jquery' ), $this->version, false );
		...
		wp_enqueue_script( $this->plugin_name.'DateTimePickerFullMinJs', plugin_dir_url( __FILE__ ) . 'js/jquery.datetimepicker.full.min.js', array( 'jquery' ), $this->version, false );
